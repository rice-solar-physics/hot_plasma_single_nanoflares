...
\begin{document}
	...
	%
	\par\hl{Histograms of cool (top row) and hot (bottom row) slopes for the entire parameter space as outlined in \autoref{fig:parameter_space}. The histograms in the top row are grouped by type of heating function such that any one curve contains all values of $T_N$, but represents only one combination $\alpha,b$. Histograms in the bottom...}
	%
	\begin{figure*}
		\centering
		\begin{minipage}[c]{0.49\textwidth}
		...
		\end{minipage}
			\caption{Clockwise from top left: cool slopes grouped by heating function for the electron heating case; cool slopes grouped by heating function for the ion heating case; hot slopes grouped by $T_N$ value for the single-fluid case; hot slopes grouped by $T_N$ for the electron heating case.}
			\label{fig:histos}
	\end{figure*}
	%
	\section{Discussion}
