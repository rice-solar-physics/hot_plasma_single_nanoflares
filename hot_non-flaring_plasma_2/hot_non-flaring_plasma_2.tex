...
\begin{document}
	...
	\begin{figure*}
		...
		\end{minipage}
			\caption{Non-equilibrium ionization $\mathrm{EM}$ (\textbf{left column}) and  $d\log{\mathrm{EM}}/d\log{T}$ (\textbf{right column}) comparisons for electron heating (\textbf{top row}), ion heating (\textbf{middle row}), and the single-fluid case (\textbf{bottom row}). In all cases, $L=40$ Mm, $\alpha=-2.5$, $\beta=1$, and $T_N=5000$ s. The fits and derivatives are all calculated in the same manner as \autoref{fig:el_em}, \autoref{fig:ion_em}, and \autoref{fig:single_em}.}
			\label{fig:teff_test_case}
	\end{figure*}
	%
	\par\hl{Discussion of non-eq. ionization test cases.}
	%
