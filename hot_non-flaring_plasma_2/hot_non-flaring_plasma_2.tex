...
\begin{document}
	...
	\begin{figure*}
		...
		\end{minipage}
		\caption{$\alpha=-2.5,~b=1,~L=40$ Mm, electron heating. \textbf{Left:} $\log{\mathrm{EM}(T)}$ as a function of $\log{T}$ for all values of $T_N$. There is an artificial spacing of $\Delta\log{\mathrm{EM}}=0.2$ between each curve for aesthetic purposes. The bold blue (red) lines on either side of the peak show the linear fit to the cool (hot) emission. \textbf{Top right:} Average slopes from linear fit to all $N_{MC}$ curves as a function of $T_N$. The error bars indicate one standard deviation from the distribution of $N_{MC}$ fits. In the case of hot slopes, the absolute value is shown. \textbf{Bottom right:} Derivative of $\log{\mathrm{EM}}$ with respect to $\log{T}$. The legend is the same as the left panel.}
		\label{fig:el_em}
	\end{figure*}
	%
	\begin{figure*}
