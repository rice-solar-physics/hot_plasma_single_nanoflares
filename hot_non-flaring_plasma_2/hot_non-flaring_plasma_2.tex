...
\begin{document}
	...
	%
	\par As the calculation of $T_{eff}$ is significantly more expensive than an EBTEL calculation, we do not compute $\mathrm{EM}_{eff}$ for the entire parameter space and instead consider only a small number of test cases. \autoref{fig:teff_test_case} shows $EM_{eff}$ and its derivative (as well as the case where non-equilibrium ionization is not considered) for all preferentially-heated species (electron, ion, single-fluid), $L=40$ Mm, $\alpha=-2.5$, and $T_N=5000$ s. For all three preferentially-heated species, we note that the cool slope is unchanged. This can be seen in both the fit to the cool side of the emission measure as well as the derivative of the emission measure.
	%
	\par \hl{TODO:electron case} As in \autoref{fig:ion_em}, we do not compute the hot slope for the case of ion heating. Looking at both the left and right panels in the middle row of \autoref{fig:teff_test_case}, we see that the inclusion of non-equilibrium ionization in the case of ion heating has little to no effect on the emission measure. \hl{TODO:single-fluid case}.
	%
	...
	\begin{enumerate}
		...
		\item\hl{TODO:NEI point}\label{itm:nei}
	\end{enumerate}
	%
	...
	%
	\par\hl{TODO:short paragraph on NEI effects on EM}
	%
	\section{Conclusions}
