...
\begin{document}
	...
	\begin{figure*}
		...
		\begin{minipage}[c]{0.49\textwidth}
			\subfigure{%
			\includegraphics[width=\columnwidth]{figures/{teff_electron_profiles}.pdf}
			\label{fig:teff_el_em}}
			\subfigure{%
			\includegraphics[width=\columnwidth]{figures/{teff_ion_profiles}.pdf}
			\label{fig:teff_ion_em}}
			\subfigure{%
			\includegraphics[width=\columnwidth]{figures/{teff_single_profiles}.pdf}
		...
		\end{minipage}
			\caption{Non-equilibrium ionization $T$ (solid), $T_{eff}$ (dashed) and $n$ (dot-dashed), profiles (\textbf{left column}) and  $\mathrm{EM}$ distributions (\textbf{right column}) for electron heating (\textbf{top row}), ion heating (\textbf{middle row}), and the single-fluid case (\textbf{bottom row}). In all cases, $L=40$ Mm and the loop is subject to a single pulse of energy $Q\approx10^{25}$ erg with a duration of $\tau_H=100$ s. The values printed on the $\mathrm{EM}$ plots are the resulting emission measure slopes for the cool (blue) and hot (red) slopes for the cases of $\mathrm{EM}$ and $\mathrm{EM}_{eff}$.}
	...
	\begin{figure}
		\centering
		\includegraphics[width=\columnwidth]{figures/{electron_ts_compare}.pdf}
		\label{fig:electron_ts}
		\caption{ Comparison between Coulomb collision timescale $\tau_{ei}$ and electron thermal conduction timescale $\tau_{C,e}$ for the single-pulse example shown in the top row of \autoref{fig:teff_test_case}. The vertical dotted line marks the end of the heating period.}
	\end{figure}
	\begin{figure}
		\includegraphics[width=\columnwidth]{figures/{electron_single_nt_compare}.pdf}
		\label{fig:ss_em}
		\caption{$n-T$ state space diagram for the single-pulse examples in \autoref{fig:teff_test_case}. In the cases of electron and ion heating, only $T_e$ is shown. The corresponding $\mathrm{EM}$ distributions are overlaid in red.}
	\end{figure}
	%
	\par To address \autoref{itm:deriv} and \autoref{itm:histos}, we note that the hot sides of the electron heating $\mathrm{EM}$ distributions in \autoref{fig:el_em} have two distinct phases at long $T_N$: the $\mathrm{EM}$ drops off steeply (compared to the single-fluid case) just after the peak and then there is a pronounced hot shoulder near $\log{T}\sim7.3$. These features are missing in the single-fluid case (\autoref{fig:single_em}) where the $\mathrm{EM}$ is nearly linear (in $\log{\mathrm{EM}},\log{T}$) hotward of the peak until the sharp cutoff near $\log{T}\sim7.4$. This is emphasized in the single-pulse $\mathrm{EM}$ distributions for electron heating, ion heating, and the single-fluid case in \autoref{fig:ss_em}. 
	%
