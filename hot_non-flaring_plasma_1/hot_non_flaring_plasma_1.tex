...
%
	...
	\begin{figure}[t]
		...
		\caption{Contributions to the EBTEL two-fluid electron energy evolution by thermal conduction, radiation, and binary Coulomb interactions. The parameters for the loop evolution are the same as those for the electron heating, $\tau=20$ s case discussed in \autoref{sec:results}. The exact expressions for each curve are found in \autoref{eq:press_e_0d_2fl_breakdown}. The black dotted line shows $\psi_{TR}$ to emphasize its importance in the derivation of the two-fluid equations.}
