...
where $F_c$ is the Spitzer-H{\"a}rm heat flux. Smaller values of $f$ limit the heat flux to a greater degree. The main aspect of inclusion of a free-streaming limit is to slow down conductive cooling. We do not consider here other conduction models \citep[e.g. the non-local model discussed in the coronal context by][]{karpen_nonlocal_1987,west_lifetime_2008} since they lead to similar generic results. 
	...
	%
	\par In the first set of results we assume the plasma behaves as a single fluid, and ignore heat flux limiting and ionization non-equilibrium. \autoref{fig:nT_tau_compare} shows temperature (upper panel) and density (lower panel) as a function of time for a single nanoflare in a loop with $2L = 80$ Mm for three different pulse duraations, $\tau$: 500, 200, 50 and 20 s. The heating function that takes the form of a triangular pulse and injects a total of 10 ergs cm$^{-3}$. The the peak heating rate is varied such that the total energy input is the same. The top left panel of \autoref{fig:em_vary_all} shows the corresponding emission measure distribution $\mathrm{EM}(T)$ curves. These parameters correspond roughly to the bright AR core loops. The temperature and value of the peak emission measure are the same in all cases.
	\begin{figure}
		\centering
	...
	The top right and bottom left panels of \autoref{fig:em_vary_all} show the effect of varying the peak heating rate $H_0$ (with fixed $\tau$ such that the total energy injected increases) and loop length $L$. In all cases the pulse length is 200 s.
	%
	\begin{figure*}
		\centering
	...
	%
	\par The red curves in the upper (lower) panel of \autoref{fig:nT_teff_compare} show $T_{eff}$ for $\tau=20\,(500)$ s for the single-fluid, electron heating, and ion heating cases. \autoref{fig:em_teff_compare} shows the corresponding $EM(T_{eff})$. The effect of NEI is to truncate  $\mathrm{EM}(T)$ to around or below 10 MK. The bump on the distribution characteristic of the heating phase is also relocated to lower temperatures.
	%
	\begin{figure}
		\centering
